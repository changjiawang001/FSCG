function [centers, Zv, Z_g] = FastmultiCLR(X, v, anchor_num, opts, isGraph)
% [centers, Z_g] = FastmultiCLR(X, v, c, anchor_num, opts, isGraph)
% Input:
...
if isGraph == 1
    ...
    if opts.style == 1 % direct sample
        % 返回 拼接X之后的锚点 XX_anchor 和 锚点位置索引 ind
        [XX_anchor,ind,~] = gen_anchor_std_en(XX,m);
        % 获取每个视角的锚点
        for v = 1:n_view
            centers{v} = X{v}(ind, :);
        end
    ...
    %%
    % disp('----------global anchor Graphs Inilization----------');
    D = L2_distance_1(XX', XX_anchor');
    [~, idx] = sort(D, 2); % sort each row
    Z_g = zeros(n,m);
    for ii = 1:n
        id = idx(ii,1:k+1);
        di = D(ii, id);
        Z_g(ii,id) = (di(k+1)-di)/(k*di(k+1)-sum(di(1:k))+eps);
    end
    ...
    for v = 1:n_view
        ...
        for ii = 1:n
            id = idx(ii,1:k+1);
            di = D(ii, id);
            Zv{v}(ii,id) = (di(k+1)-di)/(k*di(k+1)-sum(di(1:k))+eps);
        end
    end
end
